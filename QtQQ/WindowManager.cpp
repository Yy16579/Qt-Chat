...
void WindowManager::addNewTalkWindow(const int& uid) {
	...
	if (widget == nullptr) {
		...
		//	  查 ContactBook 缓存，根据 uid 判断窗口是 单聊 还是 群聊，向对应的表提取 name, sign, picture 信息
		QString windowName;
		QString windowSign;
		QPixmap windowPix;
		
		bool isGroup = ContactBook::getInstance().isGroup(uid);
		if (isGroup == true) {
			//为群聊
			DepartmentInfo depInfo = ContactBook::getInstance().departmentInfo(uid);
			windowName = depInfo.name;
			windowSign = depInfo.sign;
			windowPix.load(depInfo.picture);
		}
		else {
			//为单聊
			EmployeeInfo empInfo = ContactBook::getInstance().employeeInfo(uid);
			windowName = empInfo.name;
			windowSign = empInfo.sign;
			windowPix.load(empInfo.picture);
		}
